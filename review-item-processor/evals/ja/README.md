...
```
✓ ja/examples/floor_plan_hitl_suite.jsonから10個のテストケースをロード
✓ 3つの評価者で実験を作成

🤖 エージェント評価を実行中...
  ケース1/10を処理中: TC001-very-specific-fire-extinguisher...
  ケース2/10を処理中: TC002-specific-emergency-light-battery...
  ケース3/10を処理中: TC003-moderate-exit-signs...
  ケース4/10を処理中: TC004-general-fire-equipment...
  ケース5/10を処理中: TC005-ambiguous-safety-equipment...
  ケース6/10を処理中: TC006-absent-sprinkler-system...
  ケース7/10を処理中: TC007-absent-fire-alarm...
  ケース8/10を処理中: TC008-multi-requirement-all-valid...
  ケース9/10を処理中: TC009-specific-negative-parking-extinguisher...
  ケース10/10を処理中: TC010-quantity-verification...
...
False Positives: 2 (TC009, TC010が誤検出)

=============================================================
信頼度キャリブレーション (HITL安全性)
=============================================================
ECE (キャリブレーション誤差):   0.070 (優秀 - <0.10)
Brier Score:                    0.120
...
| メトリクス          | 値      | 意味                                               | これは良いか?                 |
| ------------------- | ------- | -------------------------------------------------- | ----------------------------- |
| **Accuracy**        | 80%     | エージェントは 10 のうち 8 つの予測を正しく行った  | ✓ 良好 - 大半の予測が正確     |
| **Recall**          | 100% ⭐ | すべての実際の違反を検出(偽陰性 0)                 | ✓ 優秀 - 本番環境で安全       |
| **Precision**       | 50%     | 4 つの"fail"予測のうち 2 つが実際の違反            | ⚠️ 改善余地あり - 誤検出が多い |
| **F1 Score**        | 0.67    | 適合率と再現率のバランス指標                       | ✓ 良好 - recall優先の結果     |
| **ECE**             | 0.070   | 信頼度スコアが精度と非常によく一致                 | ✓ 優秀 - よくキャリブレーション済 |
...
**TC001-TC005, TC008 (pass cases, 0.95 confidence):**
- ✅ 全て正解 → Accuracyに貢献
- ✅ 高信頼度で正しい → ECE (優秀なキャリブレーション) に貢献
- ✅ TC001 (非常に具体的): プロンプトが具体的であれば高精度を実証

**TC006-TC007 (evidence absent, fail cases, 0.40 confidence):**
- ✅ 正しくfailと判定 → **100% Recallに決定的に貢献**
- ✅ 低信頼度で正しい判断 → 証拠不在を適切に認識
- ✅ スプリンクラーシステム、火災警報システムの記載なしを検出

**TC009 (negative check, ❌ false positive):**
- ❌ 期待: pass、実際: fail (0.40 confidence)
- 原因: 消火器の有効期限が**暗黙的に**認証タグで示されているが、明示的な期限日付なし
- エージェントは明示的証拠を要求 → 厳格だが誤検出

**TC010 (quantity threshold, ❌ false positive):**
- ❌ 期待: pass、実際: fail (0.40 confidence)
- 原因: 非常灯の具体的な台数が明記されていない
- ドキュメントには「LED emergency lighting system」とあるが台数不明 → 検証不可として fail

#### 重要な洞察

**✅ HITL システムとして安全:**
- **100% Recall**: すべての実際の違反を検出 (TC006, TC007)
- **0 Critical Errors**: 高信頼度で違反を見逃したことがない
- **ECE 0.070**: 信頼度が実際の精度と非常によく一致

**⚠️ 改善の余地:**
- **50% Precision**: TC009, TC010が誤検出
- **原因**: 暗黙的情報 vs 明示的情報のギャップ
- **トレードオフ**: エージェントは厳格 (安全側に倒す) → 一部の人間レビューが増えるが、違反見逃しはゼロ

**80%の精度でもHITL安全な理由:**
- すべてのエラーが偽陽性 (不要な人間レビュー)
- 偽陰性ゼロ → 違反の見逃しなし
- HITL では recall > precision が正しい優先順位
