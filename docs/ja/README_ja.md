...
このソリューションは Amazon Bedrock の従量課金制を使用します。料金の大部分は Bedrock の利用料が占めるため、以下では Bedrock の料金目安を記載しています。

### 安価な軽量モデル（Claude Haiku 4.5など）
- **コンテキストウィンドウ**: 200K トークン
- **処理可能ページ数（目安）**: 約80-85ページ
- **料金例（80ページ）**: 約$0.28

### 高精度な大容量モデル（Claude Opus 4.5など）
- **コンテキストウィンドウ**: 1M トークン
- **処理可能ページ数（目安）**: 約430ページ
- **料金例（400ページ）**: 約$5.75

> [!Important]
> - 上記は基本的な処理の目安です。**実際の料金は手元のサンプルドキュメントでテストして確認してください**
> - Amazon Bedrock Converse APIではファイルサイズ4.5MBの制限があります
> - **拡張機能（Knowledge Base、Code Interpreterなど）を使用するエージェント機能が設定されたチェック項目では、コンテキストウィンドウの肥大化により最大10倍程度の料金になる可能性があります**
> - 詳細な料金とトークン使用量は審査結果画面で確認できます

最新の料金情報については [Amazon Bedrock 料金ページ](https://aws.amazon.com/bedrock/pricing/) をご確認ください。
