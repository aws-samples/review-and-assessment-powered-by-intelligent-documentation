...
export const handler: SQSHandler = async (event: SQSEvent) => {
  for (const record of event.Records) {
    ...
    try {
      console.log(
        `Starting ambiguity detection for checkListSetId: ${checkListSetId}`
      );
      ...
      const checkListSet =
        await repo.findCheckListSetDetailById(checkListSetId);
      ...
      console.log(
        `Completed ambiguity detection for checkListSetId: ${checkListSetId}`
      );
    } catch (error) {
